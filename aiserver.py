...
def get_message(msg):
    ...
    # Format - Trim incomplete sentences
        ...
        socketio.emit('from_server', {'cmd': 'setlabelnumseq', 'data': msg['data']})
        settingschanged()
    elif(msg['cmd'] == 'setwidepth'):
        vars.widepth = int(msg['data'])
        socketio.emit('from_server', {'cmd': 'setlabelwidepth', 'data': msg['data']})
        settingschanged()
...
def setStartState():
    ...
    socketio.emit('from_server', {'cmd': 'updatescreen', 'data': txt})
...
def actionretry(data):
    if(vars.noai):
        socketio.emit('from_server', {'cmd': 'errmsg', 'data': "Retry function unavailable in Read Only mode."})
        return
    if(vars.aibusy):
        return
    set_aibusy(1)
...
def generate(txt, min, max):    
    ...
    try:
        ...
        socketio.emit('from_server', {'cmd': 'errmsg', 'data': 'Error occured during generator call, please check console.'})
...
def genselect(genout):
    ...
    socketio.emit('from_server', {'cmd': 'genseqs', 'data': genout})
...
def selectsequence(n):
    ...
    refresh_story()
    socketio.emit('from_server', {'cmd': 'texteffect', 'data': len(vars.actions)})
    socketio.emit('from_server', {'cmd': 'hidegenseqs', 'data': ''})
    vars.genseqs = []
...
def refresh_settings():
    ...
    if(vars.model != "InferKit"):
        ...
        socketio.emit('from_server', {'cmd': 'updatenumseq', 'data': vars.numseqs})
    else:
        socketio.emit('from_server', {'cmd': 'updatetemp', 'data': vars.temp})
        socketio.emit('from_server', {'cmd': 'updatetopp', 'data': vars.top_p})
    ...
    socketio.emit('from_server', {'cmd': 'updatewidepth', 'data': vars.widepth})
    socketio.emit('from_server', {'cmd': 'updateuseprompt', 'data': vars.useprompt})
...
def saveas(name):
    ...
    if(not fileops.saveexists(name) or (vars.saveow and vars.svowname == name)):
        ...
        socketio.emit('from_server', {'cmd': 'hidesaveas', 'data': ''})
        vars.saveow = False
        vars.svowname = ""
    else:
        ...
        vars.saveow   = True
        vars.svowname = name
        socketio.emit('from_server', {'cmd': 'askforoverwrite', 'data': ''})
...
def save():
    ...
    if(".json" in vars.savedir):
        ...
        socketio.emit('from_server', {'cmd': 'saveas', 'data': ''})
...
#==================================================================#
def getloadlist():
    socketio.emit('from_server', {'cmd': 'buildload', 'data': fileops.getstoryfiles()})
